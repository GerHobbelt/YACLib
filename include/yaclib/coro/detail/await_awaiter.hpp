...
AwaitAwaiter::AwaitAwaiter(Cores&... cores) : _await_core{sizeof...(cores) + 1} {
  ...
  const auto wait_count = (... + static_cast<std::size_t>(cores.SetWait(_await_core, PCore::kWait)));
...
AwaitAwaiter::AwaitAwaiter(It it, std::size_t count) : _await_core{count + 1} {
  ...
  for (std::size_t i = 0; i != count; ++i) {
    wait_count += static_cast<std::size_t>(it->GetCore()->SetWait(_await_core, PCore::kWait));
    ++it;
  }
