...
void WaitGroup<InitCount, Event>::AddCore(Cores&... cores) noexcept {
  ...
  static_assert((... && std::is_same_v<detail::CCore, Cores>), "Futures must be Future in WaitGroup::Add function");
...
void WaitGroup<InitCount, Event>::AddRange(const Range& range, std::size_t count) noexcept {
  ...
  const auto wait_count = range([&](detail::CCore& core) {
    return core.SetWait(_event, detail::PCore::kWait);
  });
