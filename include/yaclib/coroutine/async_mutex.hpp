...
class AsyncMutex {
 public:
  ...
  auto Unlock(IExecutor& executor) noexcept {
    return UnlockAwaiter<Type>{*this, executor};
  }

  void UnlockHere(IExecutor& executor) noexcept {
 ...
 private:
    ...
    bool await_suspend(yaclib_std::coroutine_handle<detail::PromiseType<V, E>> handle) {
      ...
      while (true) {
        if (old_state == _mutex.NotLocked()) {
          ...
          if (_mutex._state.compare_exchange_weak(old_state, base_core,
  ...
  yaclib_std::atomic<void*> _state = NotLocked();  // locked without waiters, not locked, otherwise - head of the awaiters list
