...
class Core : public ResultCoreT<Type, Ret, E> {
 ...
 private:
  ...
  void Here(PCore& caller, [[maybe_unused]] PCore::State state) noexcept final {
    if (!this->Alive()) {
      return this->Drop();
    }
    if constexpr (Wrapper::kIsAsync) {
      if (state == PCore::kHereWrap) {
        auto& core = static_cast<ResultCore<Ret, E>&>(caller);
        return this->Done(std::move(core.Get()), [] {
        });
      }
    } else {
      YACLIB_DEBUG(state != PCore::kHereCall, "");
    }
 ...
 private:
  ...
  void CallResolveAsync(Core& self, T&& value) {
    if constexpr (Async) {
      ...
      future.GetCore().Release()->SetHere(self, PCore::kHereWrap);
    } else {
...
auto SetCallback(ResultCorePtr<Arg, E> caller, Func&& f) {
  ...
  if constexpr (CallbackT != CallbackType::On) {
    caller.Release()->SetHere(*callback, PCore::kHereCall);
  } else {
  ...
  if constexpr (CoreT == CoreType::Then) {
    ...
    callback->SetDrop(PCore::kWaitDetach);
  }
}
